...
$(window).on("load", function() {
    ...
    setTimeout(function() {
      ...
      airportCodeLookUp(cities, function() {
        ...
        localStorage.setItem(
          "hotelCodes",
          JSON.stringify(airportCodes.slice(1))
        );
        // get flights
        getFlights();
        // and get hotels
  ...
  function appendDates(origin, destination, departureDates, callback) {
    ...
    var searchurl =
      siteurl + apikey + origin + destination + depDate + type + length;
    ...
    $.ajax({
      ...
      console.log("DEPARTURE DATES LENGTH");
  ...
  function inner(city, hotelCity) {
    ...
    while (ic < JSON.parse(localStorage.getItem("hotelRanges")).length) {
      hotelstay(
        city,
        JSON.parse(localStorage.getItem("hotelRanges"))[ic],
        hotelCity
      ); /* call hotelstay with city and date */
      ic++;
    }
  ...
  function outer(hotelCities) {
    ...
    while (oc < JSON.parse(localStorage.getItem("hotelCodes")).length) {
      inner(
        JSON.parse(localStorage.getItem("hotelCodes"))[oc],
        hotelCities[oc]
      ); /* call inner loop with present city name */
      oc++;
    }
